 ...
 * Accepte aussi un format leger {start, end, text} sans words.
 *
  ...
  for (let i = 0; i < raw.length; i += 1) {
    ...
    if (Array.isArray(wordsRaw) && wordsRaw.length > 0) {
      ...
      for (const w of wordsRaw) {
        ...
        if (!text || !Number.isFinite(ws) || !Number.isFinite(we)) continue;
    ...
    const line = String(rec.text || "").trim();
    if (!line) return null;
    const tokens = splitTranscriptWords(line);
    if (!tokens.length) return null;
    const span = Math.max(end - start, 0.05);
    const slice = span / tokens.length;
    const words: TranscriptWordCue[] = tokens.map((text, wi) => ({
      text,
      start: start + wi * slice,
      end: wi === tokens.length - 1 ? end : start + (wi + 1) * slice,
    }));
    cues.push({ index: i, start, end, words });
  }
...
/**
 * Compresse les cues trop longs (silence de fin inclus par Whisper).
 *
 * @param cues Cues stockes.
 * @returns Cues recalibres pour le karaoke.
 */
export function compactOverlongCues(cues: TranscriptCue[]): TranscriptCue[] {
  ...
  for (let i = 0; i < cues.length; i += 1) {
    ...
    const expected = Math.max(0.8, n * 0.42);
    let start = Math.max(Number(cue.start) || 0, prevEnd);
    let end = Math.max(Number(cue.end) || start + expected, start + 0.05);
    if (end - start > expected * 1.55) {
      end = start + expected;
    }
    const span = Math.max(end - start, 0.05);
    const weights = cue.words.map((w) => Math.max((w.text || "").length, 1));
    const totalW = weights.reduce((a, b) => a + b, 0) || 1;
    let cursor = start;
    const words = cue.words.map((w, wi) => {
      const slice = span * (weights[wi] / totalW);
      const wStart = cursor;
      const wEnd = wi === cue.words.length - 1 ? end : cursor + slice;
      cursor = wEnd;
      return { text: w.text, start: wStart, end: wEnd };
    });
    out.push({ index: i, start, end, words });
    prevEnd = end;
  }
  return out;
}
